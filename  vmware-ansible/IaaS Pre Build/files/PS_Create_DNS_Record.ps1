...
if($forwardlookup -match "$IP4Address"){
    write-host "DNS record already exists, checking reverse lookup"
    }
Else{
    write-host "No DNS record found, attempting to create as follows: $servername $zonename $IP4Address"
...
if ($reverselookup -match "$IP4Address"){
...
exit 0
    ...
    if ($createZone -match "Command completed successfully"){
